...
namespace ngsai
{
    ...
    */
    class ThreadPool
    {
        public:
            /*!
            * \brief Split a range [from,to) into n equal non-overlapping 
            * intervals [from,b), [b,c), .... [d,to) where [d,to) is the n-th 
            * slice. This function is usefull to compute the boundary indices 
            * in between which worker threads should work on a common data 
            * structure.
            * \param from the lower range limit (comprised within the range).
            * \param to the upper range limit (non included in the range).
            * \param n the number of slices to split the range into.
            * \return a vector containing pairs [from,to) for each interval.
            */
            static std::vector<std::pair<size_t,size_t>>
            split_range(size_t from,
                        size_t to,
                        size_t n) ;
        ...
        private:
            ...
            */
            void
            debug_print(const std::string& msg,
